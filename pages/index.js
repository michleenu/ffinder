...
const LANGUAGES = ['any','english','french','japanese','korean','spanish','italian','mandarin','cantonese','hokkien','hindi','german','portuguese']
const COUNTRIES = ['any','USA','UK','France','Japan','South Korea','China','Taiwan','Hong Kong','Singapore','Italy','Spain','India','Germany','Brazil','Mexico']
...
export default function Home() {
  ...
  const [normalFilms, setNormalFilms] = useState([])
  const [indieFilms, setIndieFilms] = useState([])
  const [normalSeen, setNormalSeen] = useState([])
  const [indieSeen, setIndieSeen] = useState([])
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('filmfinder_favourites')
      if (saved) setFavourites(JSON.parse(saved))
    } catch {}
  }, [])

  function toggleFavourite(film) {
    setFavourites(prev => {
      const exists = prev.find(f => f.title === film.title)
      const next = exists ? prev.filter(f => f.title !== film.title) : [...prev, { title: film.title, year: film.year, trailer_query: film.trailer_query, watch_url: film.watch_url }]
      try { localStorage.setItem('filmfinder_favourites', JSON.stringify(next)) } catch {}
      return next
    })
  }

  function isFavourited(title) {
    return favourites.some(f => f.title === title)
  }

  const films = indieMode ? indieFilms : normalFilms

  async function fetchFilms(isRegen = false) {
    setLoading(true)
    setError('')
    const seen = indieMode ? indieSeen : normalSeen
    ...
    try {
      ...
      if (indieMode) {
        setIndieFilms(data.films)
        setIndieSeen(prev => [...prev, ...data.films.map(f => f.title)])
      } else {
        setNormalFilms(data.films)
        setNormalSeen(prev => [...prev, ...data.films.map(f => f.title)])
      }
    } catch (e) {
  ...
  const sorted = [...films].sort((a, b) => {
    ...
    if (sortBy === 'vibe') return (b.vibe_score || 0) - (a.vibe_score || 0)
    return 0
  })
      ...
      <div style={s.page}>
        ...
        <div className="app-layout">
          ...
          <main className="app-main">
            {/* Favourites panel */}
            {favourites.length > 0 && (
              <div style={s.favPanel}>
                <div style={s.favHeader} onClick={() => setShowFavourites(!showFavourites)}>
                  <span style={s.favTitle}>♥ saved films ({favourites.length})</span>
                  <span style={s.favToggle}>{showFavourites ? '▲' : '▼'}</span>
                </div>
                {showFavourites && (
                  <div style={s.favList}>
                    {favourites.map((f, i) => {
                      const trailerUrl = `https://www.youtube.com/results?search_query=${encodeURIComponent(f.trailer_query || `${f.title} ${f.year} official trailer`)}`
                      return (
                        <div key={i} style={s.favItem}>
                          <div style={s.favItemLeft}>
                            <span style={s.favItemTitle}>{f.title}</span>
                            <span style={s.favItemYear}>{f.year}</span>
                          </div>
                          <div style={s.favItemLinks}>
                            {f.watch_url && <a href={f.watch_url} target="_blank" rel="noopener noreferrer" style={s.favLink}>watch</a>}
                            <a href={trailerUrl} target="_blank" rel="noopener noreferrer" style={s.favLink}>trailer</a>
                            <button onClick={() => toggleFavourite(f)} style={s.favRemove}>✕</button>
                          </div>
                        </div>
                      )
                    })}
            ...
            <div style={s.formCard}>
              ...
              {/* Indie toggle */}
              <div style={s.toggleRow} onClick={() => setIndieMode(!indieMode)}>
                  ...
                  <div style={s.grid2}>
                    ...
                    <Field label="duration">
                      <select value={`${durationMin}-${durationMax}`} onChange={e => {
                        ...
                        <option value="0-60">&lt; 1 hr</option>
                        <option value="0-90">&lt; 1.5 hrs</option>
                        <option value="0-120">&lt; 2 hrs</option>
                        <option value="0-150">&lt; 2.5 hrs</option>
                        <option value="0-180">&lt; 3 hrs</option>
                      </select>
                    </Field>
                  </div>
            ...
            {films.length > 0 && (
              <div style={s.results}>
                ...
                {sorted.map((film, i) => <FilmCard key={film.title + i} film={film} indieMode={indieMode} isFavourited={isFavourited(film.title)} onFavourite={() => toggleFavourite(film)} />)}
...
function FilmCard({ film, indieMode, isFavourited, onFavourite }) {
  ...
  return (
    <div style={s.card}>
      <div style={s.posterWrap}>
        ...
        <button onClick={onFavourite} style={{...s.heartBtn, color: isFavourited ? '#FF6B8A' : 'rgba(255,255,255,0.9)'}}>
          {isFavourited ? '♥' : '♡'}
        </button>
      </div>
      <div style={s.cardBody}>
        ...
        {Array.isArray(film.description) ? (
          <ul style={s.cardBullets}>
            {film.description.map((pt, i) => <li key={i} style={s.cardBullet}>{pt}</li>)}
          </ul>
        ) : (
        ...
        {film.vibe_score && (
          <div style={s.vibeRow}>
            <span style={{
              ...s.vibeBadge,
              background: film.vibe_score >= 9 ? '#2A7848' : film.vibe_score >= 8 ? '#9A6800' : '#6A6A6A',
            }}>
              vibe {Number(film.vibe_score).toFixed(2)}/10
            </span>
            {film.vibe_reason && <span style={s.vibeReason}>{film.vibe_reason}</span>}
          </div>
        )}
        <div style={s.linkRow}>
          {film.watch_url && (
...
const s = {
  ...
  heartBtn: { position: 'absolute', top: 5, right: 5, background: 'rgba(0,0,0,0.45)', border: 'none', fontSize: 12, cursor: 'pointer', padding: '1px 3px', lineHeight: 1, borderRadius: 10, backdropFilter: 'blur(4px)' },
  favPanel: { background: '#FAEAF2', border: '1px solid rgba(180,80,140,0.18)', borderRadius: 12, marginBottom: 14, overflow: 'hidden' },
  favHeader: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', cursor: 'pointer' },
  favTitle: { fontSize: 13, fontWeight: 500, color: '#2D1040' },
  favToggle: { fontSize: 11, color: '#9A5070' },
  favList: { borderTop: '1px solid rgba(180,80,140,0.12)', padding: '6px 0' },
  favItem: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 14px', gap: 8 },
  favItemLeft: { display: 'flex', alignItems: 'baseline', gap: 6, minWidth: 0 },
  favItemTitle: { fontSize: 13, color: '#2D1040', fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
  favItemYear: { fontSize: 11, color: '#9A5070', flexShrink: 0 },
  favItemLinks: { display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 },
  favLink: { fontSize: 11, color: '#7A3AAA', textDecoration: 'none', fontWeight: 500 },
  favRemove: { background: 'none', border: 'none', fontSize: 12, color: '#9A5070', cursor: 'pointer', padding: '0 2px' },
  ...
  cardBullets: { fontSize: 12, color: '#5C3050', lineHeight: 1.6, marginBottom: 8, paddingLeft: 14 },
  cardBullet: { marginBottom: 3 },
  vibeRow: { display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8, flexWrap: 'wrap' },
  vibeBadge: { fontSize: 10, fontWeight: 500, color: '#fff', borderRadius: 8, padding: '2px 7px', flexShrink: 0 },
  vibeReason: { fontSize: 11, color: '#9A5070', fontStyle: 'italic', lineHeight: 1.4 },
  ...
  linkRow: { display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginTop: 4 },
